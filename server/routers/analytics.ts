...
import { users, posts, trades, wallets, proposals, nfts, orders, aiConversations } from "../../drizzle/schema";
...
export const analyticsRouter = router({
  ...
  getPlatformStats: publicProcedure.query(async () => {
    ...
    if (!db) return {
      totalUsers: 0, totalPosts: 0, totalTrades: 0, totalNFTs: 0,
      totalVolume: "0", activeProposals: 0, aiConversations: 0,
    };
    ...
    const [aiCount] = await db.select({ count: sql<number>`count(*)` }).from(aiConversations);
    const [proposalCount] = await db.select({ count: sql<number>`count(*)` }).from(proposals).where(eq(proposals.status, "active"));

    const [volumeResult] = await db.select({ total: sql<string>`COALESCE(SUM(total), '0')` }).from(trades);
